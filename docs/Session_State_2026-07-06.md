...
- ✅ **T3-3-5 报告完稿**：`finalize_s3_3_report.py`（纯本地、幂等）将三部分正文 + 验证题库统计包装为**成稿**《真实 MES 业务流程理解报告（S3-3·Phase-1 里程碑）》——新增封面/执行摘要/方法论与脱敏合规/覆盖统计表/阅读导航/「待确认」总纲（前言）+ 结论·验收关联/后续/产物位置复现/三方签字区（结语），第一部分标题降级、陈旧提示修正。关键数字由三部分小节表+题库正则抽取(状态34/追溯边13/实体键14/流程组12/题21/token 74,937/接地26/26)非硬编码；标记块幂等可反复重跑。+9 单测(kb-ingest 全套 **226 通过**)。同步刷新 `docs/Phase1_验收就绪清单.md`(S3-3→AI侧完成，待BIZ评分)。
- ✅ **S3-3 全部完成（AI 侧）**：T3-3-1~5 五子任务全交付；`docs/S3-3_业务流程理解报告.md` 成稿(2130行,四部分) + `docs/S3-3_业务流程验证题.md`(21题,接地26/26)。**唯待 BIZ 对验证题评分≥90% + TL/ITM 签字**。
- ✅ **S3-4-1 系统理解总报告（Phase-1 里程碑汇总）**：`build_system_report.py`（纯本地、无AI、每次全量重写）从 census/S3-1/S3-2/S3-3/题库/断言manifest/术语表**正则抽数汇总**（缺失回退默认，复用 finalize 抽 S3-3），产 `docs/MES真实系统理解总报告.md`（九章+两附：执行摘要/系统概览(1860表/9160单元/193万行)/P0与方法/三大理解成果/知识资产(术语9698/断言657/RAG5155chunk/脱敏0命中)/三准确率DoD/局限/排期/里程碑判定/产物清单/三方签字区）。**诚实定位**：AI 侧100%就绪，Phase-1 可签字验收唯缺三项人工评分+ITM签字。+9 单测(kb-ingest 全套 **235 通过**)。同步刷新 Phase1 清单 S3-4-1→已出。
- ✅ **T3-4-3 Token 复盘 + P1/P2 排期**：`build_token_p1p2_plan.py`（纯本地、无AI）产 `docs/S3-4-3_Token复盘与P1P2排期.md`。**复盘**：Sprint3 合计 **3.99M**（07-15 批训练 3.94M 跨 §12 降级250万/暂停300万双阈值→TL 授权临时 8M 闸留痕；07-16 回落 51K）；批训练占 96%，理解合成7.5万、验证/报告类确定性 0 token。**单位成本**：表 ~5.6K/张、子程序 ~2.5K/个、确定性任务 0。**P1/P2 分层**（按中心度 rank，非百分位TIER列2117不可用）：P0=Top100表/Top60包(1401子程序,已训)；P1=表101-400(300)+包61-200(140,~1562子程序)≈**5.6M/约2训练日**(8M闸内1日)主动训；P2=表401-2249(1849)+单元201-5101(4901)**不主动全训**，RAG兜底+按需增量(单资产~5.6K/2.5K可忽略)。+11 单测(kb-ingest 全套 **246 通过**)。刷新 Phase1 清单 P1/P2→已出。
- ✅ **T3-4-4 Phase-1 评审会材料**：`build_phase1_review.py`（纯本地、无AI，复用 S3-4-1 抽数器保口径一致）产 `docs/S3-4-4_Phase1评审会材料.md`——会议信息+80分钟议程(8议题)+交付一览+DoD六项核对(✅已达3/⏳待评分3)+三准确率核验与人工评分安排(责任人/口径)+脱敏合规(ITM)+遗留与P1P2排期+评审决议(通过/有条件通过/驳回三选一)+四方签字区+会前材料索引。**建议「有条件通过」**（AI侧100%就绪，待三项人工评分回填自动生效）。+8 单测(kb-ingest 全套 **254 通过**)。
- ⬜ **Sprint3 唯余人工/甲方环节**：召开 Phase-1 评审会(用上述材料)、甲方IT负责人签字；三项人工评分回填(SQL终评/存储过程抽检/业务流程BIZ评分)。**AI 侧 Sprint3 全部交付完毕**。

## ★ 2026-07-16 增量⑨：P1 训练准备（**卡在治理闸，未点火**）

- **触发**：用户口头「启动 P1 资产训练，评审已通过」。按 CLAUDE.md **§5.1**（不响应即时指令、须评审通过需求单）+ **§12**（P1 ~5.6M 超默认 300万，须 TL 预算授权），**未直接发起外部批跑**，只做非外发准备。
- **已做（安全、无预算消耗）**：
  - 物化 P1 清单落远程 `/home/xintong/mes-s3-data/p1/`：**表 299**（中心度 rank 101-400）+ **包 140**（rank 61-200），**剔除 P0 后零重叠**（已 comm 校验）。样例：SQM_SMP_CD/BSQM_TOT_JDG_RSLT/BSCH_BATCHA_PLT_JOB。
  - 起草 **P1 训练需求单 `docs/REQ-MES-AI-20260716-001_P1批训练需求单.md`**（范围/清单/预算估~5.6M/执行方案/验收/前置检查/授权签字栏）。
- **待两道闸（人工）**：① 该需求单 **IT 评审通过**；② **TL 授权 P1 临时预算闸并在单上留痕**（建议拆表批~1.67M默认闸内 + 过程批~3.9M须临时闸）。两闸签齐后按需求单 §E 执行（复用 P0 管线：select→自省拉源→切分→网关脱敏门 Kimi 训练→RAG入库→validate→补断言）。
- **注意**：「评审已通过」指 Phase-1 里程碑验收，≠ P1 训练需求单评审，二者不同闸。
